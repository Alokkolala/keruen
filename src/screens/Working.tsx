...
import { api, useCarriers, useOffers, useOrder, usePacedLog, usePoints, fmtDuration } from '../lib/data'
...
export default function Working() {
  ...
  // Хук до ранних возвратов: порядок хуков не должен зависеть от загрузки.
  const agentDone = !!order && order.status !== 'searching'
  const { visible: shownLog, settled: logSettled } = usePacedLog(order?.agent_log ?? [], agentDone)
  ...
  // «В процессе» держим и пока капельница не догнала: иначе чип говорит
  // «Готово», а строки ещё дораскрываются.
  const working = order.status === 'searching' || retrying || !logSettled
  ...
  return (
    <div className="screen">
      ...
      <section className="card flex items-center gap-3 p-3.5">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-[12.5px]">
            {/* Цифры появляются вместе со своей строкой в логе, иначе карточка
                показывает километры, пока лог ещё «считает маршрут». */}
            {order.distance_km && shownLog.length > 0 ? (
              <>
                <Num value={order.distance_km} fmt={(v) => v.toFixed(1)} /> км ·{' '}
                {fmtDuration(order.duration_min)}
              </>
            ) : (
              <span className="text-muted breathe">считаю маршрут…</span>
            )}
          </p>
        </div>
      ...
      <section className="card p-3.5">
          ...
          {working && (
            <li className="flex items-center gap-3 py-2.5">
              ...
              <span className="text-muted breathe text-[13px]">
                {/* Подпись идёт за видимыми строками, а не за базой —
                    иначе она забегает вперёд раскрытия. */}
                {shownLog.length === 0
                  ? 'Разбираю заявку…'
                  : shownLog.length === 1
                    ? 'Смотрю погоду на маршруте…'
                    : shownLog.length === 2
                      ? 'Считаю справедливую цену…'
                      : 'Подбираю машины…'}
              </span>
            </li>
          )}
        ...
        {order.weather && !broke && shownLog.length >= 2 && (
