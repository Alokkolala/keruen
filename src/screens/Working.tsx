...
import { useCarriers, useOffers, useOrder, usePoints, fmtDuration } from '../lib/data'
...
export default function Working() {
  ...
  useEffect(() => {
    if (order?.status === 'assigned') nav(`/result/${order.id}`, { replace: true })
  ...
  const working = order.status === 'searching'
    ...
    <div className="screen">
      ...
      <section className="card flex items-center gap-3 p-3.5">
        <div className="min-w-0 flex-1">
          ...
          <p className="tnum mt-1 text-[12.5px]">
            {order.distance_km ? `${order.distance_km} км · ${fmtDuration(order.duration_min)}` : 'считаю маршрут…'}
          </p>
        </div>
      ...
      <section className="card p-3.5">
        <div className="mb-2 flex items-center gap-2">
          ...
          <Chip tone={working ? 'yellow' : 'green'} icon={working ? undefined : 'check'}>
            {working ? 'В процессе' : 'Готово'}
          </Chip>
        </div>
        ...
        <ol>
          {log.map((s, i) => (
            ...
            >
              <span className="bg-green mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full text-white">
                <Icon name="check" size={10} strokeWidth={3} />
              </span>
              <div className="min-w-0 flex-1">
                <p className="text-[13px] leading-tight font-medium">{s.text}</p>
                {s.detail && <p className="text-muted mt-0.5 text-[11px] leading-tight">{s.detail}</p>}
              </div>
          ...
          {working && (
            <li className="flex items-center gap-3 py-2.5">
              ...
              <span className="text-muted text-[13px]">
                {log.length === 0 ? 'Разбираю заявку…' : 'Думаю дальше…'}
              </span>
            </li>
          )}
        </ol>
        ...
        {order.weather && (
          <div className="bg-chip-y mt-2 flex items-center gap-2 rounded-[12px] p-2.5">
            <Icon name="weather" size={16} className="text-yellow-ink" />
            <span className="text-[11.5px]">
              Учитываю погоду {order.weather.temp_c} °C, {order.weather.description}, топливо и обратный груз
            </span>
          </div>
        )}
      </section>
      ...
      {offers.length > 0 && (
        <section className="card p-3.5">
          ...
          {offers.map((o) => {
            ...
            return (
              <div key={o.id} className="border-line flex items-center gap-3 border-b py-2.5 last:border-0">
                ...
                <div className="text-right">
                  ...
                  <div className="text-muted text-[10.5px]">
                    {o.status === 'sent' ? 'отправлено' : o.status === 'countered' ? `просит ${money(o.counter)}` : o.status}
                  </div>
                </div>
              </div>
            )
